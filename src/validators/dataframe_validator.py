...
class CoreValidator(AbstractValidator):
    ...
    def validate(self, dataframe, spark) -> None:
        self.run_ge_rule_to_ndjson(
            dataframe=dataframe,
            rule=self.rule,
            columns=self.columns,
            dataset_name=self.dataset_name,
            rule_kwargs=self.rule_kwargs,
            output_table_name=self.output_table_name,
        )

    # Utilitaire : aplatit un résultat GE en "event" ELK-friendly

    def _ge_results_to_events(self, validation: dict, dataset_name: str, run_id: str, ts_iso: str) -> List[dict]:
        events = []
    ...
    def run_ge_rule_to_ndjson(
        ...
        *,
        ...
        out_dir = (
            os.path.join(silver + output_dir + dataset_name, output_table_name) if output_table_name else output_dir
        )
